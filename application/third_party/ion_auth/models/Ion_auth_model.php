...
class Ion_auth_model extends CI_Model
{
	...
	{
		//TO GET USER GROUP AND DEPARTMENT
		$this->load->model('public/Users_Group_model');
		$this->load->model('admin/Users_model');
		$this->load->model('admin/Department_model');
		$this->load->model('admin/Student_level_model');

		$user_info 			= $this->Users_model->get(['id' => $user->id]);
		$group_log 			= $this->Users_Group_model->join('groups','id','group_id',['user_id' => $user->id]);
		$group_val 			= $group_log[0]['group_id'];

		if($group_val == 2){
			$user_department 		= $this->Department_model->get(['department_id' => $user_info[0]['department']]);
			$user_department_name 	= $user_department[0]['department_name'];
			$user_level_name 		= null;
		}elseif($group_val == 3){
			$user_department 		= $this->Department_model->get(['department_id' => $user_info[0]['department']]);
			$user_level 			= $this->Student_level_model->get(['level_id' => $user_info[0]['user_level']]);
			$user_level_name 		= $user_level[0]['level_name'];
			$user_department_name 	= $user_department[0]['department_name'];
		}else{
			$user_level_name 		= null;
			$user_department_name	= null;
		}
