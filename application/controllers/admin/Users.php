...
class Users extends Admin_Controller {
	...
	public function create_data()
	{
		...
		$this->form_validation->set_rules('matric_no', 'Matric Number');
	...
	public function edit($id)
	{
		...
		$user_data = $this->users_model->get(['id' => $id]);
		$group_data = $this->users_group_model->get(['user_id' => $id]);
		
		/* Load Model */
		$this->load->model('admin/Department_model','dept_model');
		$this->load->model('admin/Faculty_model','faculty_model');
		
		$department = $this->dept_model->get(['department_id' => $user_data[0]['department'] ]);

		$this->data['default_department'] = $department[0]['department_id'];
		
		$this->data['default_faculty'] = $department[0]['department_faculty_id'];
		
		$this->data['all_faculties'] = $this->faculty_model->get();
		$this->data['all_department'] = $this->dept_model->get(['department_faculty_id' => $department[0]['department_faculty_id']]);
		...
		$this->data['matric_no'] = array(
			...
			'type'  => 'text',
			'class' => 'form-control',
			'value' => $this->form_validation->set_value('matric_no',$user_data[0]['matric_no']),
		);
